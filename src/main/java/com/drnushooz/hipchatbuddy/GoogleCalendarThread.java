...
class GoogleCalendarThread implements Runnable
{
	...
	private final String applicationName = "HipchatBuddy";
	private final String hipchatApiUri = "https://api.hipchat.com";
	private AtomicBoolean pausedFlag;
	private long pollIntervalMs;


	public GoogleCalendarThread(Map<String,Object> settingsMap)
	{
		...
		try
		{
			...
			dataStoreDir = new File(userHome+fileSeparator+".hipchatbuddy"); //Place where client token is saved
			if(dataStoreDir!=null && !dataStoreDir.exists())
			{
				LOG.debug("Settings directory doesn't exist. Creating at "+dataStoreDir.getAbsolutePath());
				dataStoreDir.mkdirs();
			}
			...
			LOG.debug("Created datastore factory at "+dataStoreDir.getAbsolutePath());
			Credential credential = authorize();
			service = new com.google.api.services.calendar.Calendar.Builder(httpTransport,jsonFactory,credential).setApplicationName(applicationName).build();
	...
	private Credential authorize() throws Exception
	{
		String fileSeparator = System.getProperty("file.separator");
		String userHome = System.getProperty("user.home");
		String clientSecretsPath = userHome+fileSeparator+".hipchatbuddy"+fileSeparator+"client_secrets.json";
		LOG.debug("Trying to read client_secrets.json from "+clientSecretsPath);
		InputStream googleClientToken = new FileInputStream(clientSecretsPath);
		GoogleClientSecrets clientSecrets = GoogleClientSecrets.load(jsonFactory, new InputStreamReader(googleClientToken));
		...
		GoogleAuthorizationCodeFlow flow = new GoogleAuthorizationCodeFlow.Builder(
				httpTransport, jsonFactory, clientSecrets, Collections.singleton(CalendarScopes.CALENDAR_READONLY)).setDataStoreFactory(dataStoreFactory).build();
		Credential cred = new AuthorizationCodeInstalledApp(flow,new LocalServerReceiver()).authorize(hipchatEmail);
		LOG.info("User credentials saved to "+ dataStoreDir.getAbsolutePath());
		return cred;
	}
